...
class ChatConsumer2(AsyncJsonWebsocketConsumer):
    ...
    def save_user_channel(self):
        self.user = User.objects.get(email=self.user_id)
    ...
    def save_message(self, room, user, content):
        try:
            ChatRoomParticipants.objects.get_or_create(
                Q(room__name=f'{self.user.user_uid}-{self.to_user}') | Q(room__name=f'{self.to_user}-{self.user.user_uid}'), user=self.user)
            ChatRoomParticipants.objects.get_or_create(
                Q(room__name=f'{self.user.user_uid}-{self.to_user}') | Q(room__name=f'{self.to_user}-{self.user.user_uid}'), user=self.to_user)
        except Exception as e:
            print(e)

        try:
            chatroom = ChatRoom.objects.get(
                Q(name=f'{self.user.user_uid}-{self.to_user}') | Q(name=f'{self.to_user}-{self.user.user_uid}'))
